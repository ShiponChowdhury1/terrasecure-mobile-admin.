...
const SignInPage = () => {
  ...
  const { register, handleSubmit, setValue, watch, setError, formState: { errors } } = useForm<SignInFormValues>({
  ...
  const onSubmit = (data: SignInFormValues) => {
    ...
    setTimeout(() => {
      ...
      if (data.email === 'admin@gmail.com' && data.password === '123456') {
        ...
        setError('root', {
          type: 'manual',
          message: 'Invalid email or password. Only the admin can sign in.',
        })
      }
    }, 1200)
  }
  ...
  return (
    <div className='w-full'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-6 grow'>
        ...
        {/* Root Error Message */}
        {errors.root && (
          <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl p-3 text-sm font-medium text-center animate-in fade-in slide-in-from-top-1 duration-200">
            {errors.root.message}
          </div>
        )}
